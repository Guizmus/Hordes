...
// fonction au clic sur le ? des plaintes d'un citoyen
unsafeWindow.detail_plaintes = function (nom) {
...
// modules de la vue d'ensemble de la ville
...
// module de la page des chantiers
...
unsafeWindow.sab_banque = {
	creer_div : function () {
		if (($("#generic_section div.right ul.outInv").length > 0) && ($("#sab_banque").length == 0)) {
			var retour = $("<h2>Banque</h2><ul class='tools shortTools nada cityInv stocks' id='sab_banque'><li class='clear'></li></ul>");
				...
				if (contenu_banque == null)
					$.ajax({
						...
						success : function (res) {
							...
							unsafeWindow.sab_banque.afficher_contenu();
						}
					});
			} else {
				retour.last().append("<span>Pour afficher le contenu de la banque, veuillez activer le script dans les réglages.</span>");
			}
			$("#generic_section div.right ul.outInv").after(retour);
			// $("#generic_section div.right").append(retour);
			if (contenu_banque && unsafeWindow.sab_xml_hordes.api_key)
				this.afficher_contenu();
		}
	},
	afficher_contenu : function () {
		unsafeWindow.sab_cache.set_domaine("banque");
		var contenu_banque = unsafeWindow.sab_cache.get("contenu");
		...
		$.each(items, function (x,item) {
			...
			if ($("#sab_banque ."+details_item.categorie).length == 0)
				$("#sab_banque").children().last().before($("<li class='clear'></li><li class='group "+details_item.categorie+"'>"+details_item.categorie+"</li><li class='clear'></li>"));
			$("#sab_banque ."+details_item.categorie).next().after(nouvel_objet);
		});
	}
}


// liste des items et ids/img. récupéré sur BBH. script à part.
if (true) {
...
var analyser_page = function() {
	...
	if ($("#generic_section div.right ul.outInv").length > 0)
		unsafeWindow.sab_banque.creer_div();
